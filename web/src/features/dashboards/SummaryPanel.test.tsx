...
describe("SummaryPanel", () => {
  it("shows loaded data and supports refresh", async () => {
    ...
    render(
      <SummaryPanel<number> label="Citizen summary" load={load}>
        {(value) => <span>{value} reports</span>}
      </SummaryPanel>,
    );
  ...
  it("shows an error and empty state when the initial request fails", async () => {
    const load = vi.fn().mockRejectedValue(new Error("network unavailable"));

    render(
      <SummaryPanel<number> label="Platform summary" load={load}>
        {(value) => <span>{value} users</span>}
      </SummaryPanel>,
    );
    ...
    expect(screen.getByText("No summary data is available yet.")).toBeTruthy();
  });
});
