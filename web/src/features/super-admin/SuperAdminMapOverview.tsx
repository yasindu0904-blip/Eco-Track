...
export function SuperAdminMapOverview({
  accessToken,
}: {
  accessToken: string;
}) {
  ...
  const markers = useMemo<MapMarkerFeature[]>(
    () => [
      ...incidents.map((incident) => ({
        ...
        geometry: {
          type: "Point" as const,
          coordinates: [incident.longitude, incident.latitude] as [
            number,
            number,
          ],
        },
  ...
  const load = useCallback<MapViewportChangeHandler>(
    async (nextViewport, context) => {
      ...
      try {
        ...
        setEvents(eventPage.features);
        setIncidentCursor(incidentPage.nextCursor);
        setEventCursor(eventPage.nextCursor);
        setSelectedId((current) =>
          incidentPage.items.some((item) => item.id === current) ||
          eventPage.features.some((item) => item.properties.id === current)
            ? current
            : undefined,
        );
      } catch (requestError) {
        if (!context.signal.aborted)
          setError(
            describeApiFailure(
              requestError,
              "Unable to load public map markers.",
            ).message,
          );
      } finally {
    ...
    try {
      const [incidentPage, eventPage] = await Promise.all([
        incidentCursor
          ? listPublicIncidents(
              accessToken,
              { ...viewport, limit: 100, cursor: incidentCursor },
              controller.signal,
            )
        ...
        eventCursor
          ? listPublicCleanupEventMap(
              accessToken,
              { ...viewport, limit: 100, cursor: eventCursor },
              controller.signal,
            )
          : Promise.resolve(undefined),
      ]);
      ...
      if (incidentPage) {
        setIncidents((current) => [
          ...new Map(
            [...current, ...incidentPage.items].map((item) => [item.id, item]),
          ).values(),
        ]);
        setIncidentCursor(incidentPage.nextCursor);
      }
      ...
      if (!controller.signal.aborted)
        setError(
          describeApiFailure(
            requestError,
            "Unable to load more public markers.",
          ).message,
        );
    } finally {
  ...
  return (
    <section className="super-admin-map-card" aria-label="Public map oversight">
      ...
      <p role="status">
        {loading
          ? "Loading map…"
          : `${incidents.length} incidents · ${events.length} cleanup events`}
      </p>
      {(incidentCursor || eventCursor) && (
        <button
          type="button"
          disabled={loading}
          onClick={() => void loadMore()}
        >
          {loading ? "Loading…" : "Load more public markers"}
        </button>
      )}
    </section>
  );
