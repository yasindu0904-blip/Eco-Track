...
describe("role-specific map scenarios", () => {
  test("citizen cleanup-event map recovers from an API error without requesting incidents", async () => {
  ...
  test("citizen location search frames and queries the complete two-kilometre area", async () => {
    ...
    render(<CitizenIncidentDiscovery accessToken="token" />);
    expect(screen.getByTestId("selected-location").textContent).toBe("6.9271,79.8612");
    fireEvent.click(screen.getByRole("button", { name: "Use my location" }));
