...
export function PublicCleanupEventsPage({ accessToken, initialEventId, onBack }: Props) {
  ...
  useEffect(() => { const timeout = window.setTimeout(() => { void load(); if (initialEventId) void open(initialEventId); }, 0); return () => window.clearTimeout(timeout); }, [initialEventId, load, open]);

  return <main className="public-events-shell">
    <header className="event-editor-header"><div><span>COMMUNITY CLEANUPS</span><h1>Published cleanup events</h1><p>Choose a public event to see its verified schedule, instructions, and join options.</p></div><button className="secondary" type="button" onClick={onBack}>Citizen dashboard</button></header>
