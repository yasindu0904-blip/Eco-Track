...
export function IncidentPage({ accessToken, profile, initialView = "create", initialIncidentId, onOpenCleanupEvent }: IncidentPageProps) {
          ...
          setNotification({
            ...
            message:
              error instanceof Error
                ? error.message
                : "Could not load incident categories.",
          });
      });
    return () => {
      active = false;
  ...
  function chooseFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (files.length + selected.length > 5) {
      setNotification({
        kind: "error",
        message: `You can attach up to 5 photos. Remove an existing photo before adding more.`,
      });
      event.target.value = "";
      return;
    }
    if (invalid) {
      setNotification({
        kind: "error",
        message: `${invalid.name} must be a JPEG, PNG, or WebP image no larger than 8 MB.`,
      });
      event.target.value = "";
      return;
    }
  ...
  async function submitReport(event: FormEvent<HTMLFormElement>) {
    ...
    if (!locationConfirmed) {
      setNotification({
        kind: "error",
        message: "Confirm the incident location on the map before submitting.",
      });
      return;
    }
    if (!categoryId) {
      setNotification({
        kind: "error",
        message: "Select an incident category.",
      });
      return;
    }
      ...
      if (files.length > 0 && evidence.length !== files.length) {
        evidence = await uploadIncidentEvidence(
          ...
          (completed, total, fileName) => {
            setUploadProgress(
              `Uploading ${fileName} — ${completed} of ${total} complete`,
            );
          },
        );
      ...
      setNotification(
        result.idempotentReplay
          ? {
              kind: "error",
              message:
                "This report was already submitted. The existing report is shown.",
            }
          : {
              kind: "success",
    ...
    <main className="incident-shell">
      ...
      <header className="incident-header">
        ...
        <nav aria-label="Incident navigation">
          ...
          <button
            type="button"
            className={view === "reports" || view === "detail" ? "active" : ""}
      ...
      <section className="incident-content">
                      ...
                      <label
                        ...
                        className={
                          severity === option.value
                            ? `selected severity-${option.value.toLowerCase()}`
                            : ""
                        }
                      >
                        <input
                ...
                <div className="incident-fields">
                  <label>
                    ...
                    <input
                      ...
                      placeholder="e.g. Plastic waste blocking the canal"
                    />
                  </label>
                  <label>
                    Description
                    <textarea
                      ...
                      placeholder="Describe what you saw, the size of the affected area, and any immediate hazards…"
                    />
                  </label>
                  <label>
                    Address or landmark <span>(optional)</span>
                    <input
                      maxLength={500}
                      value={addressText}
                      onChange={(event) => setAddressText(event.target.value)}
                      placeholder="e.g. Near the west entrance of Viharamahadevi Park"
                    />
                  </label>
                </div>
              ...
              <section className="incident-card">
                <div className="incident-card-title">
                  ...
                  <div>
                    ...
                    <p>
                      Use the location button or move the map beneath the center
                      pin, then confirm it.
                    </p>
                  </div>
                </div>
              ...
              <section className="incident-card">
                <div className="incident-card-title">
                  ...
                  <div>
                    ...
                    <p>
                      Optional for web. Add up to 5 JPEG, PNG, or WebP photos, 8
                      MB each.
                    </p>
                  </div>
                </div>
                <label className="incident-upload">
                  <input
                    type="file"
                  ...
                  <small>
                    Select one or several images. {5 - files.length} remaining.
                  </small>
                </label>
                {files.length > 0 && (
          ...
          <section className="incident-reports-view">
            ...
            {reportList.busy || isLoading ? (
              ...
              <div className="incident-report-grid">
                {reports.map((report) => (
                  <article key={report.id} className="incident-report-card">
                    {report.thumbnailUrl ? (
                      <img src={report.thumbnailUrl} alt="" />
                    ) : (
                      <div className="incident-photo-placeholder">
                    ...
                    <div className="incident-report-body">
                      ...
                      <div>
                        ...
                        <strong>
                          {readableStatus(report.severity as IncidentStatus)}{" "}
                          severity
                        </strong>
                      </div>
                      <button
          ...
          <section className="incident-detail-view">
            ...
            <div className="incident-detail-hero">
              <div>
                ...
                <h1>{detail.title}</h1>
                <p>Reported {formatDate(detail.reportedAt)}</p>
              </div>
              <span
                className={`incident-detail-status status-${detail.status.toLowerCase()}`}
              >
                {readableStatus(detail.status)}
              </span>
            </div>
            <div className="incident-detail-grid">
              ...
              <aside>
                <article className="incident-card">
                  <h2>Location</h2>
                  <p>{detail.addressText ?? "No address supplied"}</p>
                  <strong>
                ...
                <article className="incident-card">
                  ...
                  <dl>
                    <div>
                      <dt>Severity</dt>
                      <dd>
                        {readableStatus(detail.severity as IncidentStatus)}
                      </dd>
                    </div>
                    <div>
                      <dt>Highlighted until</dt>
                      <dd>{formatDate(detail.highlightUntil)}</dd>
                    </div>
                    <div>
                      <dt>Archive after</dt>
                      <dd>{formatDate(detail.archiveAfter)}</dd>
                    </div>
                  </dl>
                </article>
                <button
                  type="button"
                  className="incident-new-button"
                  onClick={startAnotherReport}
                >
        ...
        {view === "discover" && (
          <CitizenIncidentDiscovery
            accessToken={accessToken}
            onOpenEvent={onOpenCleanupEvent}
          />
        )}
      </section>
    </main>
