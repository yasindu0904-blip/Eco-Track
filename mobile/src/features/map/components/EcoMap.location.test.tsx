...
describe("EcoMap location fallback", () => {
  ...
  test("frames and renders the selected nearby-search radius", async () => {
    ...
    const radiusSource = renderer!.root.findByProps({ id: "eco-map-search-radius" });
    expect(radiusSource.props.data.geometry.type).toBe("Polygon");
    expect(radiusSource.props.data.geometry.coordinates[0]).toHaveLength(65);
