  ...
  return <View style={sharedStyles.card}>
    ...
    {!page ? <Text style={styles.copy}>Loading volunteers…</Text> : page.participants.length === 0 ? <Text style={styles.copy}>No joined volunteers yet.</Text> : page.participants.map((participant) => {
      ...
      return <View style={styles.card} key={participant.id}>
        ...
        {active.map((allocation) => { const session = page.sessions.find(({ id }) => id === allocation.sessionId); return <View style={styles.allocation} key={allocation.id}>
          ...
          {allocation.status === "PLANNED" ? <><Text style={styles.label}>Move to another available session</Text>{available.filter(({ id }) => id !== allocation.sessionId).map((target) => <Button key={target.id} disabled={busy} variant="secondary" label={`${target.sessionDate} ${target.startTime.slice(0, 5)}`} onPress={() => void run(() => reallocateEventParticipant(accessToken, organizationId, eventId, allocation.id, target.id))} />)}<Button disabled={busy} label="Mark attended" onPress={() => void run(() => markEventAttendance(accessToken, organizationId, eventId, allocation.id, "ATTENDED"))} /><Button disabled={busy} variant="secondary" label="Mark absent" onPress={() => void run(() => markEventAttendance(accessToken, organizationId, eventId, allocation.id, "ABSENT"))} /><Button disabled={busy} variant="danger" label="Remove allocation" onPress={() => Alert.alert("Remove allocation?", "The volunteer remains joined to the event.", [{ text: "Cancel", style: "cancel" }, { text: "Remove", style: "destructive", onPress: () => void run(() => removeEventAllocation(accessToken, organizationId, eventId, allocation.id)) }])} /></> : null}
        </View>; })}
