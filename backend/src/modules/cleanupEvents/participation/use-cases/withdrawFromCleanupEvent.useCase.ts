  ...
  return dependencies.prisma.$transaction(async (transaction) => {
    ...
    if (existing.status === "REMOVED")
      throw new ApplicationError(
        409,
        "PARTICIPANT_REMOVED",
        "This participation was removed by the event team.",
      );
    if (existing.status === "WITHDRAWN") return toParticipationDto(existing);
    if (event.lifecycleStatus !== "PUBLISHED")
      throw new ApplicationError(
        409,
        "EVENT_WITHDRAWAL_CLOSED",
        "Withdrawal is closed for this event.",
      );
    if (existing.attendanceStatus !== "UNMARKED")
      throw new ApplicationError(
        409,
        "ATTENDANCE_ALREADY_RECORDED",
        "You cannot withdraw after attendance has been recorded.",
      );
    ...
    await createNotificationRecord(transaction, {
      ...
      title: "You left a cleanup event",
      message: `Your participation in ${event.title} was withdrawn.`,
      data: {
        eventId,
        organizationId: event.organizationId,
        status: "WITHDRAWN",
