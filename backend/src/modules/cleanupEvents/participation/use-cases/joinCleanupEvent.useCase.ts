...
import {
  notifyParticipationOperations,
  requireJoinableEvent,
  toParticipationDto,
} from "../participation.support.js";
    ...
    async (transaction) => {
      ...
      if (!event)
        throw new ApplicationError(
          ...
          "The cleanup event was not found.",
        );
      ...
      if (existing?.status === "REMOVED")
        throw new ApplicationError(
          409,
          "PARTICIPANT_REMOVED",
          "You cannot rejoin this event after being removed.",
        );
      if (existing?.status === "JOINED")
        return {
          participation: toParticipationDto(existing),
          created: false,
          rejoined: false,
        };
      ...
      if (
        ...
        throw new ApplicationError(
          ...
          "This cleanup event has reached its volunteer capacity.",
        );
      }
      const now = new Date();
      ...
      await createNotificationRecord(transaction, {
        ...
        title: existing
          ? "You rejoined a cleanup"
          : "You volunteered for a cleanup",
        message: `You are volunteering for ${event.title}.`,
        data: {
          eventId,
          organizationId: event.organizationId,
          status: "JOINED",
      ...
      if (!saved)
        throw new ApplicationError(
          500,
          "PARTICIPATION_SAVE_FAILED",
          "The participation could not be loaded.",
        );
      return {
        participation: toParticipationDto(saved),
        created: !existing,
