...
function invalid(validation: {
  ...
  return new ApplicationError(
    400,
    "EVENT_PARTICIPATION_REQUEST_INVALID",
    issue
      ? `${field ? `${field}: ` : ""}${issue.message}`
      : "The participation request is invalid.",
  );
}
...
export function joinEventController(dependencies: CleanupEventDependencies) {
  return async (
    ...
    try {
      ...
      const body = emptyParticipationBodySchema.safeParse(request.body ?? {});
      ...
      response
        .status(result.created || result.rejoined ? 201 : 200)
        .json({ data: result });
    } catch (error) {
      next(error);
    }
...
export function getMyParticipationController(
...
export function withdrawFromEventController(
  dependencies: CleanupEventDependencies,
) {
  return async (
    request: Request,
...
export function listMyParticipationsController(
  dependencies: CleanupEventDependencies,
) {
  return async (
    request: Request,
    ...
    try {
      ...
      response
        .status(200)
        .json({
          data: await listMyParticipations(
            dependencies,
            request.authentication!.profile.id,
            query.data,
          ),
