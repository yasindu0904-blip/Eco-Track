  ...
  return dependencies.prisma.$transaction(
    async (transaction) => {
      ...
      if (participant.status === "REMOVED")
        return { participant: toParticipantOperationDto(participant) };
      if (participant.status !== "JOINED")
        throw new ApplicationError(
          409,
          "PARTICIPANT_NOT_ACTIVE",
          "Only a joined volunteer can be removed.",
        );
      const now = new Date();
      ...
      await transaction.auditLog.create({
        data: {
          ...
          metadata: { eventId: event.id, reason: input.reason },
        },
      });
      ...
      if (!saved)
        throw new ApplicationError(
          ...
          "The removed participant could not be loaded.",
        );
      return { participant: toParticipantOperationDto(saved) };
    },
    { timeout: 30_000 },
  );
}
