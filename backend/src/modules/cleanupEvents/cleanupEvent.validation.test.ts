...
test("draft validation accepts direct and incident-linked drafts", () => {
  const base = {
    title: "Beach cleanup",
    description: "Remove plastic from the beach",
    eventLatitude: 6.927079,
    eventLongitude: 79.861244,
  ...
  assert.equal(
    createDraftSchema.safeParse({
      ...base,
      incidentId: "07e253e8-e6f8-4f67-b82a-831a32dc0462",
    }).success,
    true,
  );
});
...
test("draft validation rejects invalid coordinates and incomplete meeting coordinates", () => {
  ...
  assert.equal(
    updateDraftSchema.safeParse({ eventLatitude: 6.9 }).success,
    false,
  );
  assert.equal(
    updateDraftSchema.safeParse({ meetingLatitude: null }).success,
    false,
  );
});
...
test("draft listing validation coerces query strings and applies safe limits", () => {
  ...
  assert.equal(
    listDraftQuerySchema.safeParse({ limit: "20", unexpected: "field" })
      .success,
    false,
  );
});
