...
function invalid(result: {
  ...
  return new ApplicationError(
    400,
    "EVENT_OPERATION_REQUEST_INVALID",
    issue
      ? `${field ? `${field}: ` : ""}${issue.message}`
      : "The event operation request is invalid.",
  );
}
...
export function getEventOperationsController(
  dependencies: CleanupEventDependencies,
) {
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      const parameters = eventOperationParametersSchema.safeParse(
        request.params,
      );
...
export function getParticipantEventUpdatesController(
  dependencies: CleanupEventDependencies,
) {
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      const parameters = participantUpdateParametersSchema.safeParse(
        request.params,
      );
      if (!parameters.success) throw invalid(parameters);
      response
        .status(200)
        .json({
          data: await getParticipantEventUpdates(
            dependencies,
            parameters.data.eventId,
  ...
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      response
        .status(201)
        .json({
          data: await addEventNote(dependencies, {
            ...parameters.data,
            ...body.data,
            ...actor(request),
          }),
...
export function createEventEvidenceUploadIntentsController(
  dependencies: CleanupEventDependencies,
) {
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      response
        .status(201)
        .json({
          data: await createEventEvidenceUploadIntents(dependencies, {
            ...parameters.data,
            ...body.data,
            actorUserId: request.authentication!.profile.id,
          }),
...
export function registerEventEvidenceController(
  dependencies: CleanupEventDependencies,
) {
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      response
        .status(201)
        .json({
          data: await registerEventEvidence(dependencies, {
            ...parameters.data,
            ...body.data,
            actorUserId: request.authentication!.profile.id,
          }),
...
export function getCompletionReadinessController(
  dependencies: CleanupEventDependencies,
) {
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      response
        .status(200)
        .json({
          data: await getEventCompletionReadiness(
            dependencies,
            parameters.data.organizationId,
            parameters.data.eventId,
          ),
  ...
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      response
        .status(200)
        .json({
          data: await cancelCleanupEvent(dependencies, {
            ...parameters.data,
            ...body.data,
            ...actor(request),
          }),
...
export function completeEventController(
  dependencies: CleanupEventDependencies,
) {
  return async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      response
        .status(200)
        .json({
          data: await completeCleanupEvent(dependencies, {
            ...parameters.data,
            ...body.data,
            ...actor(request),
          }),
