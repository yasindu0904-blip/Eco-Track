...
function releasedIncidentStatus(
  now: Date,
  incident: { highlightUntil: Date; archiveAfter: Date },
) {
...
export async function cancelCleanupEvent(
  ...
  return dependencies.prisma.$transaction(
    async (transaction) => {
      ...
      const transition = event.currentWorkflowStatus.outgoingTransitions.find(
        ({ toStatus }) => toStatus.mappedLifecycleStatus === "CANCELLED",
      );
      if (!transition)
        throw new ApplicationError(
          409,
          "EVENT_CANCELLATION_NOT_CONFIGURED",
          "The current workflow status cannot be cancelled.",
        );
      const now = new Date();
      ...
      if (changed.count !== 1)
        throw new ApplicationError(
          ...
          "The event changed in another request. Refresh and retry.",
        );
      await transaction.cleanupEventReminder.updateMany({
        where: { cleanupEventId: event.id, processedAt: null },
      ...
      if (event.incident && event.incident.status === "CLEANUP_ORGANIZED") {
        ...
        if (incidentChanged.count !== 1)
          throw new ApplicationError(
            ...
            "The linked incident changed in another request. Refresh and retry.",
          );
      ...
      const saved = await transaction.cleanupEvent.findUniqueOrThrow({
        where: { id: event.id },
