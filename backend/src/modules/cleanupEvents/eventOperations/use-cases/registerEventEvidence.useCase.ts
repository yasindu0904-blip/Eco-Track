...
export async function registerEventEvidence(
  ...
  if (!input.storagePath.startsWith(expectedPrefix)) {
    throw new ApplicationError(
      422,
      "EVENT_EVIDENCE_PATH_INVALID",
      "The evidence upload does not belong to this event and user.",
    );
  }
  if (
    !(await dependencies.eventEvidenceStorage.objectExists(input.storagePath))
  ) {
    throw new ApplicationError(
      422,
      "EVENT_EVIDENCE_MISSING",
      `The upload for ${input.originalFileName} is incomplete.`,
    );
  }
  const evidence = await dependencies.prisma.$transaction(
    async (transaction) => {
      const current = await findEventOperationsRecord(
        transaction,
        input.organizationId,
        input.eventId,
      );
      ...
      await transaction.auditLog.create({
        data: {
          ...
          entityId: created.id,
          metadata: { eventId: current.id, type: input.type },
        },
      });
      return created;
    },
