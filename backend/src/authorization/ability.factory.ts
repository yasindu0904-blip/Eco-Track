...
function hasCompletedProfile(profile: AuthenticatedUserProfile): boolean {
  return Boolean(
...
export function buildAbilityForRequest(
  ...
  const { profile, tenant, eventAuthorization } = context;

  if (profile.accountStatus !== "ACTIVE") {
    return build();
  }

  can([Actions.ReadOwn, Actions.Update], Subjects.UserProfile, {
    id: profile.id,
  });
  ...
  can([Actions.ReadOwn, Actions.MarkRead], Subjects.Notification, {
    userId: profile.id,
  });
  ...
  if (profile.platformRole === "SUPER_ADMIN") {
    ...
    can(
      [Actions.Read, Actions.Review, Actions.Approve, Actions.Decline],
      Subjects.OrganizationApplication,
    );
    can(Actions.Read, Subjects.Organization);
    can(Actions.Read, Subjects.OrganizationServiceArea);
  ...
  can(Actions.ReadOwn, Subjects.OrganizationApplication);
  can(Actions.Read, Subjects.Organization);
  can(
  ...
  if (tenant.membership.role === "ORG_ADMIN") {
    ...
    can([Actions.Read, Actions.ManageWorkflow], Subjects.CleanupWorkflow, {
      organizationId,
    });
    can([Actions.Read, Actions.Review], Subjects.IncidentReview, {
      organizationId,
    });
    can(
      [
    ...
    can([Actions.Read, Actions.AssignCoordinator], Subjects.EventCoordinator, {
      cleanupEvent: {
        is: {
          organizationId,
        },
    ...
    can(
      [Actions.RemoveParticipant, Actions.RecordAttendance],
    ...
    can([Actions.Read, Actions.AddNote], Subjects.EventNote, {
      cleanupEvent: {
        is: {
          organizationId,
        },
    ...
    can([Actions.Read, Actions.UploadEvidence], Subjects.EventEvidence, {
      cleanupEvent: {
        is: {
          organizationId,
        },
  ...
  if (
    ...
    eventAuthorization.cleanupEvent.organizationId !== organizationId
  ) {
    return build();
  }

  const cleanupEventId = eventAuthorization.cleanupEvent.id;

  can([Actions.Read, Actions.Update, Actions.Complete], Subjects.CleanupEvent, {
    id: cleanupEventId,
    organizationId,
  });
  ...
  can([Actions.Read, Actions.RemoveParticipant], Subjects.EventParticipant, {
    cleanupEventId,
  });
  can(Actions.RecordAttendance, Subjects.EventParticipant, {
    cleanupEventId,
  });
  can([Actions.Read, Actions.AddNote], Subjects.EventNote, {
    cleanupEventId,
